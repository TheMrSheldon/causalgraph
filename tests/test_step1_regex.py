"""Unit tests for regex causality identifier."""
import pytest
...
def test_identifies_explicit_causal_titles(identifier):
    posts = [_make_post(t, str(i)) for i, t in enumerate(CAUSAL_TITLES)]
    result = identifier.identify(posts)
...
def test_does_not_false_positive_on_non_causal(identifier):
...
def test_empty_input(identifier):
    assert identifier.identify([]) == []


def test_name_property(identifier):
    assert identifier.name == "regex"


def test_returns_subset_of_input(identifier):
